...
public interface TripleCombiRepository extends JpaRepository<TripleCombiEntity,Long>, ICombiRepository {
    ...
    @Query(value = "select * from triple_combi where json_contains(champion_id,?1) and json_contains(position,?2) and perk_myth_item = ?3",nativeQuery = true)
    Optional<TripleCombiEntity> findByChampionIdAndPositionAndPerkMythItem(String championId, String position, String perkMythItem);
}